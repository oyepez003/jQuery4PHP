 ...
 * @version    SVN: $Id$
 */
class YsGrid extends YsComponent{
  ...
  private $beforeSearchExpandedContent;
  private $afterSearchExpandedContent;
  private $afterEditCell;
  private $afterSaveCell;
  private $cmTemplate;
  private $subGridOptions;
  private $keyboard;
  private $useColSpanStyle;
  private $groupHeaders;
  private $idPrefix;
  private $beforeProcessing;
  private $isFrozenColumns;
  ...
  public function varsToUnset() {
    ...
    'calendarDefaultOptions','beforeSearchExpandedContent',
    'afterSearchExpandedContent', 'keyboard','groupHeaders','useColSpanStyle',
    'isFrozenColumns');
  }

  public function __construct($gridId, $caption = null, $gridHtmlProperties = null) {
    YsJQuery::usePlugin(YsJQueryConstant::PLUGIN_JQGRID);
  ...
  public function getUseColSpanStyle() {
    return $this->useColSpanStyle;
  }
  
  /**
   * Determine if the non grouping header cell should be have cell above it -
   * value of false, or the column should be treated as one combining boot -true
   * @param boolean $useColSpanStyle 
   */
  public function setUseColSpanStyle($useColSpanStyle) {
  ...
  /**
   * A set of object(s) which describes the header grouping texts and rules. 
   * Bellow is the list of the options of every element of this array
   * @param array $groupHeaders 
   */
  public function setGroupHeaders($groupHeaders) {
    if($groupHeaders !== null && is_array($groupHeaders)){
      if(isset($this->groupHeaders)){
        array_push($this->groupHeaders,$groupHeaders);
      }else{
        $this->groupHeaders = array($groupHeaders);
      }
    }
  }
  ...
  public function getAfterEditCell() {
    return $this->afterEditCell;
  }

  public function setAfterEditCell($afterEditCell) {
    $this->afterEditCell = $afterEditCell;
  }

  public function getAfterSaveCell() {
    return $this->afterSaveCell;
  }

  public function setAfterSaveCell($afterSaveCell) {
  ...
  public function addGridFields() {
    ...
    foreach ($args as $gridField) {
      if ($gridField instanceof YsGridField) {
        ...
        if($gridField->getFrozen() === true){
          $this->isFrozenColumns = true;
        }
      } else {
  ...
  public function build() {
    ...
    $this->getYsJQGrid()->condition(YsJQuery::elementExists('#gview_'.$this->getGridId()));
  ...
  private function buildColModel() {
    ...
    if (sizeof($fields) > 0) {
      ...
      $i = 0;
      foreach ($fields as $field) {
        $this->setGroupHeaders($field->getGroupHeaders());
  ...
  private function buildPostSintax() {
    ...
    if (isset($this->keyboard) || $this->keyboard !== null) {
      $this->getYsJQGrid()->addPostSintax($this->keyboard->render($this->getJQuerySelector(), $this->getPager()));
    }
    if (isset($this->groupHeaders) || $this->groupHeaders !== null){
       $this->getYsJQGrid()->addPostSintax($this->invokeDestroyGroupHeader());
       $this->getYsJQGrid()->addPostSintax($this->buildGroupHeaderSintax());
    }
    if ($this->isFrozenColumns===true){
       $this->getYsJQGrid()->addPostSintax($this->invokeDestroyFrozenColumns());
       $this->getYsJQGrid()->addPostSintax($this->invokeSetFrozenColumns());
       $this->getYsJQGrid()->addPostSintax($this->reloadGrid());
    }
    $this->getYsJQGrid()->addPostSintax($this->getPostSintaxAsArray());
  }
  
  private function buildGroupHeaderSintax(){
    return $this->invokeSetGroupHeaders($this->getUseColSpanStyle(),$this->getGroupHeaders());
  }
  ...
  public function searchExpandedContentIn($url,$args = null) {
    $this->setSubGridRowExpanded($this->subGridRowExpandedFunction($url,$args));
  }

  public function setBeforeSearchExpandedContent($function) {
    $this->beforeSearchExpandedContent = $function;
  }

  public function getBeforeSearchExpandedContent($function) {
    return $this->beforeSearchExpandedContent;
  }

  public function setAfterSearchExpandedContent($function) {
    if(is_string($function)){
      $this->afterSearchExpandedContent = new YsJsFunction($function,'response, status, xhr');
    }else{
      $this->afterSearchExpandedContent = $function;
    }
  }

  public function getAfterSearchExpandedContent($function) {
    return $this->afterSearchExpandedContent;
  }

  private function subGridRowExpandedFunction($url,$args = null) {
    $dataStr = '';
    if($args !== null){
      foreach($args as $key => $val){
        $dataStr .= sprintf("data['%s'] = %s;", $key, YsJSON::valueToJS($val));
      }
    }
    $beforeFunction = (isset($this->beforeSearchExpandedContent))
                      ? $this->beforeSearchExpandedContent : "";
    $afterFunction = (isset($this->afterSearchExpandedContent))
                      ? $this->afterSearchExpandedContent 
                      : new YsJsFunction("return false");
    $afterFunction = YsJSON::valueToJS($afterFunction);
    $bodySyntax = <<<EOH
    $beforeFunction
    var data = {subgrid:subgridid, rowid:id};
      ...
      $dataStr
      $("#"+jQuery.jgrid.jqID(subgridid)).load('$url',data,$afterFunction);
EOH;
...
EOH;
  ...
  public function showInUserData($userData, $align, $userDataVarName = 'userdata') {
    $sintax = new YsJQueryDynamic(
                    YsJQuery::css("text-align", $align)->in($this->getGridTSelector())->setPreSintax(sprintf('var %s = %s', $userDataVarName, $this->invokeGetUserData())),
  ...
  public function setColumnModel(YsGridColumnModel $columnModel){
    $this->setCmTemplate($columnModel->render());
  }
  ...
  public function setSubGridExpandOnLoad($value){
    $this->addSubGridOption("expandOnLoad", $value);
  }
  
  public function setSubGridPlusIcon($value){
    $this->addSubGridOption("plusicon", $value);
  }
  
  public function setSubGridMinusIcon($value){
    $this->addSubGridOption("minusicon", $value);
  }
  
  public function setSubGridOpenIcon($value){
    $this->addSubGridOption("openicon", $value);
  }

  public function setSubGridReloadOnExpand($value){
    $this->addSubGridOption("reloadOnExpand", $value);
  }
  
  public function setSubGridSelectOnExpand($value){
    $this->addSubGridOption("selectOnExpand", $value);
  }
  
  public function addSubGridOption($key,$value){
    $subGridOptions = ($this->subGridOptions !== null && is_array($this->subGridOptions)) ? $this->subGridOptions  : array(); 
    $this->subGridOptions = array_merge($subGridOptions,array($key => $value));
  }
  
  public function invokeGetPostData(){
    return $this->invoke('getGridParam', 'postData');
  }
  
  public function invokeSetPostData($data){
    return $this->invoke('setGridParam', $data);
  }
  
  public function invokeAppendPostData($data){
    return $this->invokeSetPostData($data);
  }
  
  public function invokeSetPostDataItem($key, $val){
    return $this->invoke('setGridParam', array('postData' => array($key => $val)));
  }
  
  public function invokeGetPostDataItem($key){
    return $this->invoke('getGridParam', 'getGridParam')->setAccesors(sprintf('[%s]',$key));
  }
  
  public function invokeRemovePostDataItem($key){
    return $this->invokeGetPostDataItem($key);
  }
  
  public function invokeGetUserData(){
    return $this->invoke('getGridParam', 'userData');
  }
  
  public function invokeGetUserDataItem($key){
    return $this->invoke('getGridParam', 'userData')->setAccesors(sprintf('[%s]',$key));
  }
  
  public function invokeSetGroupHeaders($useColSpanStyle, $groupHeaders){
    return $this->invoke('setGroupHeaders',array('useColSpanStyle' => $useColSpanStyle,'groupHeaders' => $groupHeaders));
  }
  
  public function invokeDestroyGroupHeader($boolean = null){
    return $this->invoke('destroyGroupHeader', $boolean);
  }
  
  public function invokeDestroyFrozenColumns($boolean = null){
    return $this->invoke('destroyFrozenColumns', $boolean);
  }

  public function invokeSetFrozenColumns(){
    return $this->invoke('setFrozenColumns');
  }
  
  public function reloadGrid($options = null){
    return YsJQGrid::reloadGrid($this->getJQuerySelector(), $options);
  }
  ...
  public function setMaxPerPageList($args){
    $args = func_get_args();
    $this->setRowList($args);
  }
}
...
class YsGridField extends ArrayObject implements ArrayAccess{
  ...
  private $groupHeaderStartColumnName;
  private $groupHeaderNumberOfColumns; 
  private $groupHeaderTitleText;
  ...
  private function varsToUnset(){
    return array( 'colname','gridFormatter','groupHeaderStartColumnName',
    'groupHeaderNumberOfColumns','groupHeaderTitleText'
    );
  }
  ...
  public function getGroupHeaderStartColumnName() {
    return $this->groupHeaderStartColumnName;
  }

  public function setGroupHeaderStartColumnName($startColumnName) {
    $startColumnName = ($startColumnName instanceof YsGridField) ? $startColumnName->getName() : $startColumnName;
    $this->groupHeaderStartColumnName = $startColumnName;
  }

  public function getGroupHeaderNumberOfColumns() {
    return $this->groupHeaderNumberOfColumns;
  }

  public function setGroupHeaderNumberOfColumns($numberOfColumns) {
    $this->groupHeaderNumberOfColumns = $numberOfColumns;
  }

  public function getGroupHeaderTitleText() {
    return $this->groupHeaderTitleText;
  }

  public function setGroupHeaderTitleText($titleText) {
    $this->groupHeaderTitleText = $titleText;
  }
  
  public function setGroupHeaders($numberOfColumns,$titleText){
    $this->setGroupHeaderStartColumnName($this->getName());
    $this->setGroupHeaderNumberOfColumns($numberOfColumns);
    $this->setGroupHeaderTitleText($titleText);
  }
  
  public function groupColumns($numberOfColumns,$titleText){
    $this->setGroupHeaders($numberOfColumns,$titleText);
  }
  
  public function getGroupHeaders(){
    if(isset($this->groupHeaderTitleText) && isset($this->groupHeaderNumberOfColumns) && isset($this->groupHeaderStartColumnName)){
      return array(
        'startColumnName' => $this->getGroupHeaderStartColumnName(),
        'numberOfColumns' => $this->getGroupHeaderNumberOfColumns(),
        'titleText' => $this->getGroupHeaderTitleText()
      );
    }else{
      return null;
    }
  ...
  public function setCollspan($count){
    $this->setCellAttributes(sprintf("colspan=%s",$count));
  }
  
  public function displayNone($template = null){
    $this->setCellAttributes("style=display:none");
  }
  
  public static function configureCellAttributes($attrs, $template = null){
    if($template === null){
      $template = sprintf("function(rowId, value, rowObject, colModel, arrData) { return ' %s '; }",$attrs);
    }
    return YsArgument::value($template);  
  }
  
  public function setCellAttributes($attrs){
